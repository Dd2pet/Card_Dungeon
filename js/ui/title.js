const TitleSystem = (() => {
  ...
  function _buildSlots() {
    ...
    container.innerHTML = slots.map((slot, i) => {
      if (slot.exists) {
        ...
        const clsName = GameConfig.classes[clsKey]?.label || clsKey;
  ...
  function _buildClassPicker() {
    ...
    clsRow.innerHTML = Object.entries(GameConfig.classes).map(([key, cls]) =>
      `<button class="cls-btn" data-class="${key}"><span class="cls-ic">${cls.ico}</span>${cls.label}</button>`
    ).join('');
  ...
  function _bindFormButtons() {
    UISystem.$('create-hero-btn').addEventListener('click', () => {
      ...
      const stats = { hp: classCfg.hp, mp: classCfg.mp, atk: classCfg.atk, def: classCfg.def, spd: classCfg.spd, crit: classCfg.crit };
